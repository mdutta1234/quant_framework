...
class MultiAssetTradingEnv(gym.Env):
    ...
    def __init__(self, df: pd.DataFrame, predictions_df: pd.DataFrame, config: dict):
        super().__init__()
        ...
        # Merge market data and precomputed predictions
        self.tickers = sorted(df['ticker'].unique().tolist())
        self.num_assets = len(self.tickers)
        self.dates = sorted(df['date'].unique().tolist())
        self.total_steps = len(self.dates)
        
        # To avoid massive config bloat, we pull ALL available features from the DF, 
        # plus dynamically calculated predictor & position features.
        ...
        # 2. Predictor Derived (pred_ret, residual, risk_adj, direction)
        # 3. Position Features (weight, prev_weight, pnl, holding_duration)
        self.per_stock_dim = len(self.market_feature_cols) + 4 + 4
        
        # Global Portfolio:
        # cash_ratio, ret_1d, ret_5d, ret_20d, vol_20, sharpe_20, drawdown, total_exposure, turnover
        self.global_dim = 9
    ...
    def _prepare_data_matrices(self, df: pd.DataFrame, predictions_df: pd.DataFrame):
        """Pre-aligns multi-asset market data and predictions into fast 3D numpy arrays."""
        ...
        for t_idx, date in enumerate(self.dates):
            ...
            for a_idx, ticker in enumerate(self.tickers):
                if ticker in date_data.index:
                    ...
                    self.price_matrix[t_idx, a_idx] = row['close']
                    self.pred_return_matrix[t_idx, a_idx] = row['pred_return']
                    # Fallback to 1% if rolling_std isn't present to prevent DivByZero
    ...
    def reset(self, seed=None, options=None):
        ...
        self.current_step = 60 # Start after a warmup window so rolling 60d metrics are valid
    ...
    def step(self, action: np.ndarray):
        ...
        current_portfolio_value = self.cash + np.sum(self.stock_positions * current_prices)
        
        # 1. Normalize and Constrain Actions
        ...
        step_return = (new_portfolio_value - current_portfolio_value) / (current_portfolio_value + 1e-8)
        ...
        # 5. V1 Reward Calculation (Math-Strict Component Assembly)
        log_return = np.log((new_portfolio_value + 1e-8) / (current_portfolio_value + 1e-8))
        ...
        if self.reward_cfg.get('normalize_reward', True):
            ...
            reward = np.clip(reward * 100, -clip_val, clip_val) # Scale up log returns for PPO stability

        terminated = bool(new_portfolio_value <= 0.50 * self.initial_cash) # 50% loss threshold
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        current_portfolio_val = self.cash + np.sum(self.stock_positions * prices)
        ...
        for i in range(self.num_assets):
            ...
            # 3. Position Features
            weight = (self.stock_positions[i] * prices[i]) / (current_portfolio_val + 1e-8)
            prev_wt = self.prev_weights[i]
            pnl = (prices[i] - self.entry_prices[i]) / (self.entry_prices[i] + 1e-8) if self.entry_prices[i] > 0 else 0.0
            duration = self.holding_durations[i] / 252.0 # Normalized by trading year
        ...
        cash_ratio = self.cash / (current_portfolio_val + 1e-8)
        ...
        drawdown = (current_portfolio_val - self.max_portfolio_value) / (self.max_portfolio_value + 1e-8)
        ...
        # Replace NaNs or Infs that might arise from edge case math
        full_obs = np.nan_to_num(full_obs, nan=0.0, posinf=0.0, neginf=0.0)
        return full_obs.astype(np.float32)

    def _get_info(self) -> dict:
        """The agent doesn't see these for training, but we use them for evaluation."""
        val = self.portfolio_values[-1]
        dd = (val - self.max_portfolio_value) / (self.max_portfolio_value + 1e-8)
